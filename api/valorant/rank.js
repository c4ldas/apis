  ...
  try {
    ...
    async function sendMessage(player, elo, pontos, posicao, vitorias) {
      ...
      //if (!channel || channel == '${channel}') {
        ...
        //console.log(yellow(`${new Date().toLocaleTimeString('en-UK')} - ${finalMessage} - ${noChannel}`))
        //return
      //}

      res.status(200).send(finalMessage)
      console.log(green(`${new Date().toLocaleTimeString('en-UK')} - Channel: ${channel} - ${finalMessage}`))
    }
    ...
    if (error.message.includes('Unexpected token < in JSON at position 0')) {
      ...
      console.log(red(`${new Date().toLocaleTimeString('en-UK')} - Valorant Rank API - Channel: ${channel} - ${error}`))
      return
    }
